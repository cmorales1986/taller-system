    ...
    const vehiculo = await prisma.vehiculos.findUnique({
      ...
      include: {
        ...
        ordenes_reparacion: {
          ...
          take: 10,
          include: { usuarios_ordenes_reparacion_asignado_aTousuarios: { select: { nombre: true } } }
        }
      }
    });
...
export async function PUT(
  ...
  try {
    ...
    const vehiculo = await prisma.vehiculos.update({
      where: { id: params.id },
      data: { marca, modelo, anio: anio ? parseInt(anio) : null, color, vin, kilometraje: kilometraje ? parseInt(kilometraje) : null, notas }
    });
