...
interface OrdenDetalle {
  ...
  trabajo_realizado: string | null; notas: string | null;
  kilometraje: number | null; total: number; subtotal: number;
  fecha_prometida: string | null; fecha_entrega: string | null; creado_en: string;
...
function BotonFactura({ ordenId }: { ordenId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/facturas?orden_id=${ordenId}`)
      ...
      .then((facturas: { id: string; orden_id: string }[]) => {
        ...
        if (factura) { setTieneFactura(true); setFacturaId(factura.id); }
  ...
  if (tieneFactura && facturaId) {
    return (
      <Button onClick={() => router.push(`/facturas/${facturaId}`)} variant="outline"
  ...
  return (
    <Button onClick={() => router.push(`/ordenes/${ordenId}/facturar`)}
...
export default function OrdenDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-4 lg:space-y-6 pb-10">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-xl lg:text-2xl font-bold">OR-{String(orden.numero).padStart(4, "0")}</h1>
            <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${ESTADOS[orden.estado]?.color}`}>
          ...
          {(orden.estado === "listo" || orden.estado === "entregado") && <BotonFactura ordenId={id} />}
        </div>
      </div>
                ...
                return (
                  <div key={est} className="flex items-center">
                    <div className="flex flex-col items-center">
                      <button
                        ...
                        className={`w-8 h-8 lg:w-9 lg:h-9 rounded-full text-xs font-bold transition-all border-2 ${
                          esActual ? "bg-orange-500 border-orange-500 text-white scale-110 shadow-md"
                          : esPasado ? "bg-green-500 border-green-500 text-white hover:scale-105"
                          : "bg-background border-border text-muted-foreground hover:border-orange-300"
                        }`}
                      >
                        {esPasado ? "✓" : i + 1}
                      </button>
                      <span className={`text-xs mt-1.5 text-center leading-tight w-14 ${
                        esActual ? "text-orange-600 font-semibold"
                        : esPasado ? "text-green-600"
                    ...
                    {i < ORDEN_ESTADOS.length - 1 && (
                      <div className={`h-0.5 w-8 lg:w-12 mb-5 mx-1 transition-colors ${i < estadoActualIndex ? "bg-green-400" : "bg-border"}`} />
                    )}
                  </div>
                );
              })}
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {estadoActualIndex < ORDEN_ESTADOS.length - 1 && (
                  <Button onClick={() => cambiarEstado(ORDEN_ESTADOS[estadoActualIndex + 1])}
                    disabled={actualizando} className="bg-orange-500 hover:bg-orange-600 text-white">
                    {actualizando ? "Actualizando..." : `Avanzar a "${ESTADOS[ORDEN_ESTADOS[estadoActualIndex + 1]]?.label}"`}
                  </Button>
                )}
                <Button variant="outline" onClick={() => cambiarEstado("cancelado")} disabled={actualizando}
      ...
      {/* ── CLIENTE · VEHÍCULO · DETALLES — 1 col mobile, 3 desktop ── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
        <Card>
          ...
          <CardContent className="space-y-1">
            ...
            {orden.clientes.telefono && <p className="text-sm text-muted-foreground">{orden.clientes.telefono}</p>}
            {orden.clientes.email && <p className="text-sm text-muted-foreground break-all">{orden.clientes.email}</p>}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            <span className="bg-foreground text-background text-xs font-bold px-2 py-1 rounded">{orden.vehiculos.patente}</span>
            <p className="font-semibold pt-1">{orden.vehiculos.marca} {orden.vehiculos.modelo}</p>
            <p className="text-sm text-muted-foreground">{orden.vehiculos.anio} · {orden.vehiculos.color}</p>
            {orden.kilometraje && <p className="text-sm text-muted-foreground">{orden.kilometraje.toLocaleString()} km</p>}
          </CardContent>
        </Card>

        <Card className="sm:col-span-2 lg:col-span-1">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs uppercase tracking-widest text-orange-500 font-bold">Detalles</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1">
            {orden.usuarios_ordenes_reparacion_asignado_aTousuarios && (
              <p className="text-sm text-muted-foreground">
                <span className="font-medium text-foreground">Mecánico:</span> {orden.usuarios_ordenes_reparacion_asignado_aTousuarios.nombre}
              </p>
            )}
            {orden.fecha_prometida && (
              <p className="text-sm text-muted-foreground">
                <span className="font-medium text-foreground">Entrega est.:</span> {new Date(orden.fecha_prometida).toLocaleDateString("es-PY")}
              </p>
            )}
            <p className="text-2xl font-bold pt-2">{Number(orden.total).toLocaleString("es-PY")} Gs.</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Problema reportado</p>
            <div className="bg-muted rounded-lg p-3 text-sm border border-border">{orden.descripcion_problema}</div>
          </div>
          <Separator />
          <div>
            <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2 block">Diagnóstico del mecánico</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2 block">Trabajo realizado</label>
          ...
          <Button onClick={guardarNotas} disabled={actualizando} className="bg-orange-500 hover:bg-orange-600 text-white">
            ...
            {orden.or_repuestos.length === 0 ? (
              ...
              <div>
                ...
                <div className="flex justify-between pt-3">
                  ...
                  <span className="font-bold">{orden.or_repuestos.reduce((acc, r) => acc + Number(r.subtotal), 0).toLocaleString("es-PY")} Gs.</span>
                </div>
              </div>
            )}
          </CardContent>
            ...
            {orden.or_servicios.length === 0 ? (
              ...
              <div>
                ...
                <div className="flex justify-between pt-3">
                  ...
                  <span className="font-bold">{orden.or_servicios.reduce((acc, s) => acc + Number(s.subtotal), 0).toLocaleString("es-PY")} Gs.</span>
                </div>
              </div>
            )}
          </CardContent>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        {h.estado_anterior ? (
                          <>
                            <span className={`text-xs px-2 py-0.5 rounded-full border ${ESTADOS[h.estado_anterior]?.color}`}>{ESTADOS[h.estado_anterior]?.label}</span>
                            <span className="text-muted-foreground text-xs">→</span>
                            <span className={`text-xs px-2 py-0.5 rounded-full border ${ESTADOS[h.estado_nuevo]?.color}`}>{ESTADOS[h.estado_nuevo]?.label}</span>
                          </>
                        ) : (
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${ESTADOS[h.estado_nuevo]?.color}`}>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {new Date(h.creado_en).toLocaleString("es-PY")}{h.usuarios && ` · ${h.usuarios.nombre}`}
                      </p>
                    </div>
                  </div>
                  {i < orden.or_historial.length - 1 && <Separator className="mt-4" />}
                </div>
              ))}
            </div>
          )}
